...
public class AccidentController {
    ...
    List<Object[]> getAccidentTop() {
        ...
        String sql = "select " + "c.brand as brand, " + "c.model as model, " + "count(*) as count " + "from car c left outer join accident_cars " + "on c.id = accident_cars.cars_id " + "group by c.brand, c.model";
    ...
    AccidentsDTO getAccidents(@PathVariable Long vin) {
        return AccidentsDTO.builder().accidentDTOs(accidentRepository.accidentsByVinNumber(vin).stream().map((accident -> {
            return AccidentDTO.builder().vinNumbers(accident.getCars().stream().map(Car::getVinNumber).toList()).build();
        })).toList()).build();
    }
}
